package com.supcon.mes.module_beacon.customUI;
...
public class DigitalEditText extends LinearLayout implements TextWatcher {
    ...
    public DigitalEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textSizeSp = ta.getInteger(R.styleable.DigitalEditText_edTextSize, DEFAULT_TEXT_SIZE);//(int) ta.getDimension(R.styleable.DigitalEditText_edTextSize, DEFAULT_TEXT_SIZE);
        textColor = ta.getColor(R.styleable.DigitalEditText_edTextColor, DEFAULT_TEXT_COLOR);
    ...
    private void init(Context context) {
        for (int i = 0; i < editNumber; i++) {
            ...
            edit.setTag(i);
            edit.setMaxLines(1);
            edit.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT, 1));
