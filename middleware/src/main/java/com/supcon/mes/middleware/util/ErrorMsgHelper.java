...
public class ErrorMsgHelper {
    public static boolean checkNetworkError(String errorMsg) {
        if(errorMsg.contains("服务不存在，未部署或ip设置错误！")){
            return true;
        }
        else if(errorMsg.contains("访问被拒绝，模块未授权！")){
            return true;
        }
        else if(errorMsg.contains("参数传递错误！")){
            return true;
        }
        else if(errorMsg.contains("服务器错误")){
            return true;
        }
        else if(errorMsg.contains("WIFI已断开!"))
        {
            return true;
        }
        return false;
    ...
    public static String msgParse(String errorMsg){
        ...
        else if(errorMsg.contains("401") || errorMsg.equals("No message available")){
            EventBus.getDefault().post(new LoginInvalidEvent());
            msg2= "登陆失效,重新登陆中!";
        }
